...
description: Explaining how to ensure correctness of the execution trace
keywords:
  - docs
  - zk rollups
  - polygon
...
![Figure 4: The Generic State Machine as an Algebraic Processor](figures/gen4-sm-alg-processor.png)

The vertical gray box (with the "+" sign) in the above figure, denotes addition. It expresses forming linear combinations of some of the columns; $\texttt{FREE}$, $\texttt{A}$, $\texttt{B}$, or $\texttt{CONST}$. Each is either included or excluded from the linear combination depending on whether their corresponding selectors have the value $\mathtt{1}$ or $\mathtt{0}$. An extra register denoted by $\texttt{op}$ acts as a carrier of intermediate of the computation being executed and waiting to be placed in the correct output register (on the right of Figure 4 above), depending on the values of $\texttt{setA}$ and $\texttt{setB}$. 

See next section for an illustration of how this processor enables testing the constraints.
...
![Figure 6: The PIL description for the 4-instruction program](figures/gen6-pil-4instrct-prog.png)
